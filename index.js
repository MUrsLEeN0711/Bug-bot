...
//Telegram: @MURSLEEN_OWNER
//GitHub: @MURSLEEN_OWNER
...
async function checkMembership(chatId, senderId, isRootBot) {
  ...
  while (retries > 0) {
    try {
      ...
      retries--;
      if (retries === 0) {
        await userBot.sendMessage(chatId, `❌ Failed to verify membership due to an error. Please try /checkmembership later or contact @MURSLEEN_OWNER.`);
        membershipCache.set(cacheKey, { isMember: false, timestamp: Date.now() });
        return { isMember: false };
      }
      await sleep(1000);
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'close') {
        ...
        if (!fs.existsSync(`${sessionDir}/creds.json`)) {
          ...
          const pairingCode = await sock.requestPairingCode(formattedNumber, 'MURSLEEN');
    ...
    if (mention) {
      await userSock.relayMessage(target, {
        ...
        additionalNodes: [{
          tag: "meta",
          attrs: {
            is_status_mention: " Telegram: @MURSLEEN_OWNER"
          },
          content: undefined
        }]
      });
...
📩 Please contact the developer to buy: @MURSLEEN_OWNER
...
✅ Script no enc 100%: 100$`);
    ...
    if (!isValidPhoneNumber(rawNumber)) {
      return userBot.sendMessage(chatId, '❌ Invalid phone number. Use international format (e.g., +92×××××).');
    }
...
✅ Script no enc 100%: 100$`);
  ...
  if (!isValidPhoneNumber(rawTarget)) {
    ...
    return userBot.sendMessage(chatId, '❌ Invalid target number. Use international format (e.g., +92××××××).');
  }
  ...
  const blockedNumbers = ['923134761322', '923340072073', '923156981069'];
...
└────────────────────────┘`,
    ...
    await userBot.sendMessage(chatId, `✅ Successfully started call spam to @${numberTarget} using /callspam. Pause for 2 minutes to avoid bans.`, {
      parse_mode: 'Markdown',
      reply_markup: {
        inline_keyboard: [[{ text: 'Contact Support', url: 'https://t.me/MURSLEEN_OWNER' }]],
      },
    });
...
└────────────────────────┘`, { parse_mode: 'HTML' });
  ...
  botInstance.on('callback_query', async (callbackQuery) => {
    ...
    try {
      ...
      let caption;
      let buttons = [[{ text: '〢Contact', url: 'https://t.me/MURSLEEN_OWNER' }]];
